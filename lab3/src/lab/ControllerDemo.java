...
public class ControllerDemo implements Controller {
    ...
    /**
     * Before updating the body this method check if the snake are
	 * going outside the frame, if it can take any bonus and if it
	 * is going to collide with itself and change the direction
	 * accordingly.
     */
	public void step(ArrayList<Rectangle> body) {
		...
		Rectangle right = new Rectangle(first.x + Snake.boxSize,
			first.y, GamePanel.getDimension().width, Snake.boxSize);
		Rectangle up = new Rectangle(first.x, 0, Snake.boxSize, first.y);
		Rectangle down = new Rectangle(first.x, first.y + Snake.boxSize,
			Snake.boxSize, GamePanel.getDimension().height);

		for (int k = 0; k < snake.getBonus().size(); k++) {
			if (left.intersects(snake.getBonus().get(k))) {
			...
			if (right.intersects(snake.getBonus().get(k))) {
				if ((direction == KeyEvent.VK_UP) ||
					(direction ==  KeyEvent.VK_DOWN)) {
			...
			if (up.intersects(snake.getBonus().get(k))) {
			...
			if (down.intersects(snake.getBonus().get(k))) {
				if ((direction == KeyEvent.VK_LEFT) ||
					(direction ==  KeyEvent.VK_RIGHT)) {
		...
		for (int i = 0; i < body.size(); i++) {
			if (newFirst.intersects(body.get(i)))
				direction = oldDir;
		}
